...
export const vehicleSlice = createSlice({
  ...
  reducers: {
    setDefaultVehicleId: (
      ...
      storage.global.setSync("defaultVehicleId", params.payload)
    },
    setVehicles: (
      state,
      params: {
...
export const vehicleMethods = {
  Init: createAsyncThunk(modelName + '/Init', async (
    _, thunkAPI) => {
    const dispatch = thunkAPI.dispatch


    dispatch(vehicleSlice.actions.setDefaultVehicleId(
  ...
  GetVehicles: createAsyncThunk(modelName + '/GetVehicles', async (
    {
      type = "All",
      pageNum
    }: {
    ...
    if (vehicle.loadStatus === 'loading' || vehicle.loadStatus === 'noMore') return
    ...
    if (res.code === 200 && res.data?.list?.length) {
      const tempList = (vehicle.pageNum === 1 ? [] : vehicle.vehicles).concat(res.data.list)
    ...
    if (state.vehicle.vehicles.length) {
      const { vehicle } = state
      dispatch(vehicleSlice.actions.setDefaultVehicleId(
        vehicle.vehicles.filter(v => (v.id || "") === vehicle.defaultVehicleId).length >= 1
