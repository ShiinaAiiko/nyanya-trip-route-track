...
const Cache = ({ show }: { show: boolean }) => {
  ...
  const getSize = async () => {
    ...
    const simplifyTripPositions = await storage.simplifyTripPositions.getAll()
    ...
    const s =
      ...
      JSON.stringify(simplifyTripPositions).length +
  ...
  const clearCache = async () => {
    ...
    await storage.simplifyTripPositions.deleteAll()
    await storage.global.delete('getTripStatisticsStartTime')
    ...
    await storage.global.delete('GetAllCitiesVisitedByUser')
    await storage.global.delete('GetAllCitiesVisitedByUser' + jmId)
  }
...
const About = ({ show }: { show: boolean }) => {
  ...
  const [openEngineDP, setOpenEngineDP] = useState(false)

  const [webVersionList, setWebVersionList] = useState<WebVersion[]>([])

  const [staticMode, setStaticMode] = useState('Cloud')

  let isInApp = config.devTrip ? true : nyanyaJSBridge?.isInApp()
                          ...
                          selectvalue: async (e) => {
                            ...
                            if (
                              ...
                              switchResourcesAlert(
                                ...
                                async () => {
                                  ...
                                  const res =
                                    await nyanyaJSBridge?.switchResources(
                                      e.detail.value === 'Cloud'
                            ...
                            if (downloadUrl) {
                              switchResourcesAlert(
                                ...
                                async () => {
                                  const res =
                                    await nyanyaJSBridge?.updateLocalWebResources(
                                      downloadUrl
                                    )
                                  if (res?.error) {
                                    snackbar({
              ...
              <div className="vi-right">
                {isInApp && config.appConfig?.availableEngines?.length ? (
                  <saki-dropdown
                    ...
                    ref={bindEvent({
                      close: () => {
                        setOpenEngineDP(false)
                      },
                    })}
                  >
                    <SakiButton
                      onTap={() => {
                        setOpenEngineDP(true)
                      }}
                      margin="6px 0"
                      type="Normal"
                      border="none"
                      ...
                      <saki-menu
                        ref={bindEvent({
                          selectvalue: async (e) => {
                            setOpenEngineDP(false)
                            alert({
                              title: t('switchEngine', {
                                ns: 'prompt',
                              }),
                              content: t('switchResourcesContent', {
                                ns: 'prompt',
                                content:
                                  e.detail.value === 'gecko'
                                    ? t('geckoView', {
                                        ns: 'prompt',
                                      })
                                    : t('systemWebview', {
                              ...
                              onCancel() {},
                              async onConfirm() {
                                const res = await nyanyaJSBridge?.switchEngine(
                                  e.detail.value
                                )
                                if (res?.error) {
                                  snackbar({
