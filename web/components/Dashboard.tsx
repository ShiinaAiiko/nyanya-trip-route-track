  ...
  return (
    ...
    >
      ...
      {/* <div
        ...
        className="dashboard-item dashboard-roadIcon"
      >
        <RoadIcon
          type="primary"
          country="China"
        ...
        <div className="data-position">
          <div>
            ...
            {!live && position?.timestamp ? (
              <>
                ...
                <span>
                  ...
                  <span> - </span> */}
                  {moment(Number(listenTime)).format(
                    povMode ? 'YYYY.MM HH:mm:ss' : 'YY.MM.DD HH:mm:ss'
                  )}
                </span>
              </>
            ) : (
            ...
            {!live && povMode ? (
              <>
                <span> - </span>
                <span>数据来自 trip.aiiko.club</span>
              </>
            ) : (
              ''
            )}
...
export const RoadIcon = ({
  ...
  if (country === 'CN' || cnShortCityNameList.includes(shortCityName)) {
    if (type !== 'motorway' && roadCode) {
      switch (roadCode.slice(0, 1)) {
        case 'G':
          type = 'trunk'
          break
        case 'S':
  ...
  let unnamedRoad = roadCode === 'A404'
  if (unnamedRoad) {
    type = 'unclassified'
    roadCode = ''
    roadName = t('unnamedRoad', {
      ns: 'tripPage',
    })
  }

  useEffect(() => {
    if (type === 'motorway' && !roadName && !Object.keys(tempRoadInfo).length) {
      eventListener.on('roadInfo', () => {
        setTempRoadInfo(roadInfo)
      })
      getHighwayInfo()
    }
  }, [])
  ...
  if (type === 'motorway' && !roadName && Object.keys(tempRoadInfo).length) {
    roadName =
      (tempRoadInfo?.[roadCode]?.name as any)?.[i18n.language as any] ||
      tempRoadInfo?.[roadCode]?.name?.['zh-CN'] ||
      ''
  }
  return (
    <div
    ...
    >
      {type === 'motorway' ? (
        <div className={'ri-desc ' + (roadCode.includes('S') ? 's' : 'g')}>
          {roadCode.includes('S') ? shortCityName + '高速' : '国家高速'}
        </div>
      ) : (
        ''
      )}
      {roadCode ? (
        <div className={'ri-code ' + (roadCode?.length > 3 ? 'long' : 'short')}>
          <span>{roadCode.slice(0, 3)}</span>
          <span>{roadCode.slice(3, roadCode.length)}</span>
        </div>
      ) : (
        ''
      )}
