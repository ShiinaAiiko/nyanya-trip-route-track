...
export const startTest = (bridge: NyaNyaWebJSBridge) => {
  ...
  console.log('🚗 开始车机数据模拟测试 carData')
  ...
  const generateCarData = (): CarData => {
    return {
      ...
      airQuality: {
        ...
        pm25CheckStateOut: 1,
        pm25LevelIn:
          state.pm25ValueIn < 35 ? 0 : state.pm25ValueIn < 75 ? 1 : 2,
        pm25LevelOut:
          state.pm25ValueOut < 35 ? 0 : state.pm25ValueOut < 75 ? 1 : 2,
      ...
      charge: {
        ...
        chargingRestTimeMinute: Math.floor(
          ((100 - state.elecPercentage) % 2) * 60
        ),
  ...
  const simulateDriving = () => {
    ...
    if (state.isDriving) {
      ...
      // 消耗电量
      state.elecPercentage = Math.max(
        state.elecPercentage - Math.random() * 0.01,
        5
      )
      ...
      if (state.elecPercentage < 100) {
        state.elecPercentage = Math.min(
          state.elecPercentage + Math.random() * 0.05,
          100
        )
      }
    }
    ...
    if (timeInSeconds > 6 * 3600 && timeInSeconds < 18 * 3600) {
      ...
      state.lightIntensity =
        500 + Math.sin((timeInSeconds / 3600) * Math.PI - Math.PI / 2) * 400
    } else {
      // 黑夜
  ...
  const testInterval = setInterval(() => {
    frameCount++
    // console.log('carData frameCount', bridge.getEventNames(), frameCount)
