...
class GeckoApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const GeckoContainer(),
    );
  }
}

class GeckoContainer extends StatefulWidget {
  const GeckoContainer({super.key});

  @override
  State<GeckoContainer> createState() => _GeckoContainerState();
}

class _GeckoContainerState extends State<GeckoContainer> with WidgetsBindingObserver {
  GeckoViewController? _webController;
  ...
  void _startSensorBridge() {
    _sensorTimer = Timer.periodic(const Duration(milliseconds: 1000), (_) {
      if (_webController != null) {
        ...
        ''';
        _webController?.evaluateJavascript(jsonData);
      }
    });
  }
